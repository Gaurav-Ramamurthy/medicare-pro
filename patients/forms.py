...
class PatientForm(forms.ModelForm):
    ...
    def clean_photo(self):
        """Validate photo file size and type using Pillow (no imghdr)."""
        ...
        if hasattr(photo, "size") and photo.size > MAX_PHOTO_BYTES:
            raise ValidationError(
                f"Image is too large (max {MAX_PHOTO_BYTES // (1024*1024)} MB)."
            )
        ...
        try:
            ...
            if img.format not in ("JPEG", "JPG", "PNG"):
                raise ValidationError(
                    "Uploaded file doesn't look like a valid JPEG or PNG image."
                )
        except ValidationError:
            raise
        except Exception:
            raise ValidationError("Uploaded file doesn't look like a valid image.")
